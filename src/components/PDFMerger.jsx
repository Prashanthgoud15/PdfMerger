...
const PDFMerger = () => {
  ...
  const [fileName, setFileName] = useState('prashanth-merged-document');
  ...
  const shareResult = async () => {
    const text = `I just merged ${files.length} PDFs using Prashanth's PDF Merger.`;
  ...
  return (
    <div className={`min-h-screen ${bgColor} ${textColor} flex flex-col transition-colors duration-300`}>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-2xl">
          ...
          <div className="text-center mb-12 animate-fade-in">
            <h1 className="brand-title text-4xl sm:text-5xl font-semibold tracking-tight mb-2">
              PDF Merger
            </h1>
            <p className={`${secondaryText} text-sm`}>
              No sign-up required • Private • Fast 
            </p>
          </div>
      ...
      <footer className={`w-full py-6 text-center ${borderColor} border-t`}>
        ...
        >
          Built by <span className="font-medium">Prashanth Goud</span>
        </a>
      </footer>
